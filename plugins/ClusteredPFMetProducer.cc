...
#include "CMGTools/HiggsAna2l2v/interface/FastJetAlgoWrapper.h"
...
void ClusteredPFMetProducer::produceMVAMet(edm::Event& iEvent, const edm::EventSetup& iSetup, float sumEt) {  
  ...
  //std::vector<std::string> triggerPaths=dilObjConfig_["Trigger"].getParameterSet("triggerPaths").getParameterNames();
  std::vector<std::string> allTriggerNames;
/*
  for(size_t it=0; it<triggerPaths.size(); it++)
    {
      std::vector<std::string> itriggers=dilObjConfig_["Trigger"].getParameterSet("triggerPaths").getParameter<std::vector<std::string> >( triggerPaths[it] );
      //test
...
*/
  ...
  try{
    if(hEle.isValid() && hEle->size()>0)
      {
	std::vector<reco::CandidatePtr> selElectrons   = getGoodElectrons(hEle, hMu, vtxH_, *beamSpot, hConversions, 0, lazyTool,0,pfCandsH_,lRho, dilObjConfig_["Electrons"], iSetup, lIds,allTriggerNames);
  ...
  try{
    if(hMu.isValid() && hMu->size()>0)
      {
	std::vector<reco::CandidatePtr> selMuons        = getGoodMuons(hMu, selVertices[0], lRho, dilObjConfig_["Muons"], iSetup, lIds,allTriggerNames);
