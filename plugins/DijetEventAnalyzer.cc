...
#include "CMGTools/HiggsAna2l2v/interface/TSelectionMonitor.h"
...
//
class DijetEventAnalyzer : public edm::EDAnalyzer 
{
public:
  DijetEventAnalyzer(const edm::ParameterSet &iConfig);
...
//
DijetEventAnalyzer::DijetEventAnalyzer(const edm::ParameterSet &iConfig)
...
//
int DijetEventAnalyzer::addPidSummary(ObjectIdSummary &obj)
{
...
//
void DijetEventAnalyzer::saveMCtruth(const edm::Event &event, const edm::EventSetup &iSetup)
{
  ZZ2l2nuSummary_t &ev = summaryHandler_.getEvent();
  ev.nmcparticles=0;
...
//
void DijetEventAnalyzer::analyze(const edm::Event &event, const edm::EventSetup &iSetup) 
{
  try{
    ...
    reco::VertexRef primVertex = selVertices[0]; //0: the highest Sigma_{track} pT^2 is choosen when multtiple PV's
...
*/
    ...
    if(iErr_<10) std::cout << "[DijetEventAnalyzer][analyze] failed with " << e.what() << std::endl;
...
//
void DijetEventAnalyzer::beginLuminosityBlock(const edm::LuminosityBlock&lumi, const edm::EventSetup & setup ) 
{
...
//
void DijetEventAnalyzer::endLuminosityBlock(const edm::LuminosityBlock & iLumi, const edm::EventSetup & iSetup)
{
...
//
void DijetEventAnalyzer::beginRun(const edm::Run & iRun, const edm::EventSetup & iSetup) 
{
  bool changed(true);
...
DEFINE_FWK_MODULE(DijetEventAnalyzer);
